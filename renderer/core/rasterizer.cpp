...
namespace rasterizer {
        ...
        py::array_t<float, py::array::c_style> np_depth_map)
    {
        if (np_face_vertices.ndim() != 4) {
            throw std::runtime_error("(np_face_vertices.ndim() != 4) -> true");
        }
        if (np_depth_map.ndim() != 3) {
            throw std::runtime_error("(np_depth_map.ndim() != 3) -> true");
        }
        if (np_face_index_map.ndim() != 3) {
            throw std::runtime_error("(np_face_index_map.ndim() != 3) -> true");
        }

        int batch_size = np_face_vertices.shape(0);
        int num_faces = np_face_vertices.shape(1);
        int image_height = np_depth_map.shape(1);
        int image_width = np_depth_map.shape(2);
