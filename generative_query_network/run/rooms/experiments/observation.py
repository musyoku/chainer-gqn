...
def main():
    ...
    for n in range(args.num_views_per_scene):
        ...
        x = n % sqrt_n
        y = n // sqrt_n
        figure.add(axis, x * axis_width, y * axis_height, axis_width, axis_height)
        axes_observations.append(axis)
    for n in range(args.num_views_per_scene):
        axis = gqn.imgplot.image()
        x = n % sqrt_n
        y = n // sqrt_n
        figure.add(axis, x * axis_width + 0.5, y * axis_height, axis_width, axis_height)
    ...
    with chainer.using_config("train", False), chainer.using_config(
        ...
        while True:
            ...
            query_viewpoints = xp.broadcast_to(
                query_viewpoints,
                (args.num_views_per_scene, ) + query_viewpoints.shape)
            ...
            for n in range(args.num_views_per_scene):
                ...
                r = cf.sum(r, axis=1)
                r = cf.broadcast_to(r,
                                    (args.num_views_per_scene, ) + r.shape[1:])
                ...
                for m in range(args.num_views_per_scene):
